...
export const VoiceRecorder = ({ chatId, onSent }: VoiceRecorderProps) => {
  ...
  return (
    <>
      {expanded === 'recording' && (
        <div className="w-full basis-full order-first flex items-center gap-2 py-2">
      ...
      {expanded === 'preview' && (
        <div className="w-full basis-full order-first flex items-center gap-2 p-2 rounded-xl bg-[#2d2d2f] border border-white/10 min-w-0 overflow-hidden">
      ...
      <div className="shrink-0">
        {!expanded && (
          <button
            type="button"
            className="p-2 rounded-full text-[#86868a] hover:text-white hover:bg-white/10 transition-colors touch-none"
            title="Голосовое сообщение. Удерживайте для записи."
